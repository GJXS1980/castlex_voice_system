...
int run_asr(UserData *udata)
{
	...
	snprintf(asr_params, MAX_PARAMS_LEN - 1, 
		...
		while(aud_src == 10)
		{
		printf("音频数据在哪?( Where is the audio data?)\n0: 从文件读入(Read from file)\n1: 从MIC说话(Speak from MIC)\n");
			...
			if (aud_src == 48)
			{
				...
				// printf("5.去立体仓库放置物料\n");
				// printf("6.去装配台放置物料\n");
				printf("请输入音频路径(Please enter the audio path)\n");
				// aud_src = getchar();
				// 选择音频
				...
				// 	case 49:
				// 		// 播放去装配台音频
				const char * voice2_path = voice1_path.data();
				demo_file(voice2_path, asr_params); 
				// system("play '/home/ffd/catkin_ws/src/castlex_voice_system/bin/wav/zztzq.wav'")
				// sleep(2);
				// aud_src =10;
