...
string FILE_path("/bin/bnf/castlex_com.bnf");
...
{
	printf(path1.data());
	ros::init(argc, argv, "commen_order_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	...
	commen_pub = n.advertise<std_msgs::Int32>("/Commen_CMD_Topic", 1);		// 发布离线命令词识别结果话题
	cmd_pub = n.advertise<std_msgs::Int32>("/voice/Commen_state_topic", 1);	//识别离线命令词成功的flag话题
