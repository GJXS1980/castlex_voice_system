...
class XML_Analysis():
    def __init__(self):
        ...
        self.file_path = rospy.get_param("~file_path", "/params/castlex_cmd.xml")
        ...
        print("唤醒机器人后，可以说‘前进’、‘后退’、‘左转’、‘右转’以控制机器人")
    ...
    def execute(self):
        ...
        if key in self.moveBindings.keys():
            ...
            th = self.moveBindings[key][2]
        elif key in self.speedBindings.keys():
            speed = self.speedBindings[key][0]
            turn = self.speedBindings[key][1]

        cmd_vel.linear.x = x*self.speed
        cmd_vel.linear.y = y*self.speed
        cmd_vel.angular.z = th*self.turn

        print("x:{},y={},th={}".format(cmd_vel.linear.x,cmd_vel.linear.y,cmd_vel.angular.z))
