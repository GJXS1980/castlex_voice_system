...
class Qbh_Gui():
    ...
    def run(self):
        ...
        appid = "551d49a3"
        apikey = "9635efd068d94e061f89553e421f6a7a"
        curtime = str(int(time.time()))

        param = {'engine_type':"afs",
        "aue":"raw",
        'sample_rate':"16000"
        }

        base64_param = base64.urlsafe_b64encode(json.dumps(param).encode('utf-8'))
        tt = str(base64_param).encode('utf-8')
        m2 = hashlib.md5()
        ...
        res = requests.post(url,headers= header,data = self.music_data)
        #res = requests.post(self.url,headers= header,data = json.dumps(self.music_data) )
        self.result = res.json()
        self.result_str = res.content()
        print(type(self.result))
        self.context_sub.delete(1.0, "end")
        if self.result.get('code') == '0':
            song = self.result.get('data')[0].get('song')
            print(song)
            singer = self.result.get('data')[0].get('singer')
            print(singer)
            text_ = u"歌名：" + song.encode('utf-8') +u'\n' +u"歌手："+singer.encode('utf-8')
            self.context_sub.insert("end",text_)
