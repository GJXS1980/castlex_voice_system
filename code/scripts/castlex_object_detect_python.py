...
class OBJECT_XML_Analysis():
    def __init__(self):
        self.result, self.color_id, self.result_confidence_data = None, None, None
        self.cmd_flag = False
        ...
        self.pub = rospy.Publisher('/voice/object_color', Int64, queue_size = 1)  #   发布离线命令词识别的命令词话题
    ...
    def cmd_callback(self, data):
        ...
        #   识别颜色物体
        self.color_id = self.id_data("<color", "</color>")
    ...
    def Process_Speech_cmd_to_Speed(self):
        if (self.position_id == 306):
            ...
            if self.result_confidence_data > 40:
                #   发布命令词识别结果
                self.pub.publish(self.color_id)
                print ('小谷：好的，收到!')
